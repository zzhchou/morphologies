...
#            print i, j
    ...
    cax = ax.matshow(correlations, vmin=0, vmax=1, cmap='gist_rainbow')
    fig.colorbar(cax)
    ...
    for j in range(6, 10):
        for i in range(0, 6):
            ...
            for k in range(len(weights)):
                H[:,k] *= weights[k]
            plt.imshow(H, interpolation='nearest', origin='low', extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]], cmap='viridis', vmin=0, vmax=H_max)
            plt.colorbar()
