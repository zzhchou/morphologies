...
# Main
file_list_1 = glob.glob('/home/zzchou/Documents/Data/neuron_nmo/claiborne/CNG version/*.swc')
...
file_list_2 = glob.glob('/home/zzchou/Documents/Data/test_output/morphology_recreated*.swc')
distributions_file = '/home/zzchou/Documents/Codes/zmorph_tester_distributions.txt'
...
rejections_list = [0]*num_distributions
...
for ii in range (0, num_trials):
    ...
    compartment_list_1, branch_list_1, bifurcation_list_1, termination_list_1 = read_file(file_list_1[i1])
    compartment_list_2, branch_list_2, bifurcation_list_2, termination_list_2 = read_file(file_list_2[i2])
    morphology_list_1 = [Morphology(compartment_list_1, branch_list_1, bifurcation_list_1, termination_list_1)]
    morphology_list_2 = [Morphology(compartment_list_2, branch_list_2, bifurcation_list_2, termination_list_2)]
...
#    all_terminations_list = all_terminations_list + termination_list
    ...
    for bif in bifurcation_list_1:
        if bif.parent_branch != []:
            if bif.parent_branch.parent_bifurcation != []:
                pair_table_1[bif.parent_branch.parent_bifurcation] = bif

    for bif in pair_table_1:
        id_pairs.append((bif.bifurcating_compartment.compartment_id, pair_table_1[bif].bifurcating_compartment.compartment_id))
        ordered_pairs.append((bif, pair_table_1[bif]))

    for branch in branch_list_1:
        total_dendritic_length = total_dendritic_length + branch.pathlength
        
    for bif in bifurcation_list_2:
        if bif.parent_branch != []:
            if bif.parent_branch.parent_bifurcation != []:
                pair_table_2[bif.parent_branch.parent_bifurcation] = bif

    for bif in pair_table_2:
        id_pairs.append((bif.bifurcating_compartment.compartment_id, pair_table_2[bif].bifurcating_compartment.compartment_id))
        ordered_pairs.append((bif, pair_table_2[bif]))

    for branch in branch_list_2:
        total_dendritic_length = total_dendritic_length + branch.pathlength    
    all_total_dendritic_length.append(total_dendritic_length)

    distribution_list_1 = []
    distribution_list_2 = []
    distribution_type_list_1 = []
    distribution_type_list_2 = []
    entries_list_1 = []
    entries_list_2 = []
    save_distributions(compartment_list_1, branch_list_1, bifurcation_list_1, termination_list_1, morphology_list_1, distribution_list_1, distribution_type_list_1, entries_list_1)
    save_distributions(compartment_list_2, branch_list_2, bifurcation_list_2, termination_list_2, morphology_list_2, distribution_list_2, distribution_type_list_2, entries_list_2)

    for jj in range(0, len(distribution_list_1)):
        distribution_1 = entries_list_1[jj]
        distribution_2 = entries_list_2[jj]
        ...
        if distribution_1 != distribution_2:
            ...
            pvalue = 1
        if pvalue < 0.05:
            rejections_list[jj] += 1
            
del(distribution_list_1[-1])
del(distribution_list_1[-1])            
bar_x = range(0, len(distribution_list_1))

threshold_line = [62]*len(distribution_list_1)
#rejections_list
fig, ax = plt.subplots()
plt.bar(bar_x, rejections_list[0:len(distribution_list_1)])
plt.plot([-1] + bar_x + [len(distribution_list_1) + 1], [62] + threshold_line + [62], color='black', linestyle='dashed')
ax.set_xticks(np.arange(len(distribution_list_1)))
#ax.set_xticklabels(distribution_list_1, rotation=90)
ax.set_xticklabels([x + 1 for x in bar_x])
plt.ylabel('Number of Rejections')
plt.xlim(-1, len(distribution_list_1))
plt.ylim([0, 100])
...
#save_distributions(all_compartments_list, all_branches_list, all_bifurcations_list, all_terminations_list, all_morphologies_list, distribution_list, distribution_type_list, entries_list_1)
#save_distributions(all_compartments_list, all_branches_list, all_bifurcations_list, all_terminations_list, all_morphologies_list, distribution_list, distribution_type_list, entries_list_2)
